...
A 30-second portrait animation of NanoClaw’s host/agent message loop, inspired by the fine-line mechanical diagrams on @krishnachaytanya’s TikTok account.
...
To regenerate the MP4: `npm ci`, install FFmpeg, then `node scripts/render.cjs`. This renders exactly 900 frames and synthesizes the same audio score used by the player. No remote services or agent calls are made.
